...
def prepare_data_for_targeting_insertion(*, data: List[Dict], campaign_type: str) -> List[Dict]:
    ...
    for item in data:
        model_data = {
            ...
            'campaign_name': item.get('campaignName'),
            'campaign_id': item.get('campaignId'),
            'keyword_id': item.get('keywordId') if campaign_type == AdProduct.SPONSORED_PRODUCTS.value else item.get('targetingId'),
            'keyword_type': item.get('keywordType') if campaign_type == AdProduct.SPONSORED_PRODUCTS.value else item.get('targetingType'),
            'keyword_bid': item.get('keywordBid'),
