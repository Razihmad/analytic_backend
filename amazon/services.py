...
def get_asin_categorization_by_sales(*, user_id: int, amazon_seller_id: int, start_date: str, end_date: str) -> Tuple[List, List, List]:
    ...
    for _, data in asin_wise_sales.items():
        data["cvr"] = round(float(data["orders"]) / data["total_sessions"], 2) if data.get("total_sessions") else 0
        data["tacos"] = round(float(data["ads_spend"]) / data["sales"]) if data.get("sales") else 0
        data["acos"] = round(float(data["ads_spend"]) / data["ads_sales"]) if data.get("ads_sales") else 0
